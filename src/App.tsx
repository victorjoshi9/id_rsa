...
const VoiceTeacherPopup = ({ 
  ...
  const handleStartClass = () => {
    ...
    generateSpeech(text);
    sendPushNotification('Teacher Joined', { body: 'Priyanka has joined the class and is ready to teach.' });
  };
    ...
    const interval = setInterval(async () => {
      setAnalysisProgress(prev => {
        if (prev >= 100) {
          ...
          sendPushNotification('Resources Analysis Complete', { body: 'AI has successfully found and analyzed global academic resources for you.' });
          return 100;
        }
        return prev + 2;
      });
...
export default function App() {
  ...
  const handleFindTopics = async () => {
    ...
    try {
      ...
      setTopics(result);
      sendPushNotification('Topics Generated', { body: 'AI has successfully generated thesis topics for you.' });
    } catch (error) {
  ...
  const handleGenerateThesis = async (topic: ThesisTopic) => {
    ...
    try {
      ...
      sendPushNotification('Thesis Generation Complete', { body: `Your thesis "${topic.title}" has been successfully generated by AI.` });
    } catch (error) {
